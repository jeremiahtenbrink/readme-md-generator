...
const {
  getPackageJson, getProjectName, getAuthorWebsiteFromGithubAPI, getPackageManagerFromLockFile
} = require('./utils')
 ...
 */
const getReposUrl = async packageJson => (await getReposUrlFromPackageJson(packageJson)) ||
  getReposUrlFromGit()
 ...
 */
const isGithubRepository = repositoryUrl => !isNil(repositoryUrl) &&
  repositoryUrl.includes(GITHUB_URL)
 ...
 * @param {string} repositoryUrl
 */
const getGithubUsernameFromRepositoryUrl = repositoryUrl => repositoryUrl.replace(GITHUB_URL, '')
  .split('/')[ 0 ]
...
const getProjectInfos = async () => {
  const spinner = ora('Gathering project infos')
    .start()
  
  const packageJson = await getPackageJson()
  const isJSProject = !!packageJson
  const packageManager = isJSProject ? getPackageManagerFromLockFile() : undefined
  ...
  const contributingUrl = repositoryUrl ? getContributingUrlFromRepositoryUrl(repositoryUrl) :
    undefined
  const documentationUrl = isGithubRepos ? getReadmeUrlFromGithubRepositoryUrl(repositoryUrl) :
    undefined
  const githubUsername = isGithubRepos ? getGithubUsernameFromRepositoryUrl(repositoryUrl) :
    undefined
  const authorWebsite = githubUsername ? await getAuthorWebsiteFromGithubAPI(githubUsername) :
    undefined
  const licenseUrl = isGithubRepos ? getLicenseUrlFromGithubRepositoryUrl(repositoryUrl) : undefined
